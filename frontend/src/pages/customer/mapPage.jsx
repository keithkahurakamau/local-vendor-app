...
function MapPage() {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 p-4 sm:p-6 lg:p-8 flex flex-col">
      <div className="flex-1 bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden flex flex-col relative h-[calc(100vh-2rem)]">
        
        {/* --- Toolbar --- */}
        <div className="bg-white border-b border-gray-100 p-4 z-20 flex flex-col sm:flex-row gap-4 items-center justify-between shadow-sm">
          <form onSubmit={handleSearch} className="flex-1 w-full max-w-2xl flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-500 focus:bg-white transition-all caret-orange-500"
                disabled={loading}
              />
            </div>
            <button type="submit" className="px-5 py-3 bg-orange-600 text-white font-bold rounded-xl hover:bg-orange-700 transition-colors shadow-sm disabled:opacity-50" disabled={loading}>
              {loading ? '...' : 'Search'}
            </button>
            <button type="button" onClick={handleFindNearby} className="px-4 py-3 bg-orange-50 text-orange-600 border border-orange-200 font-medium rounded-xl hover:bg-orange-100 transition-colors flex items-center gap-2 whitespace-nowrap" disabled={loading}>
              <FiNavigation />
          ...
          <div className="bg-gray-200 rounded-full p-1.5 flex items-center relative shadow-inner w-64 h-14 select-none cursor-pointer border border-gray-200">
            ...
            <div onClick={() => setViewMode('grid')} className={`flex-1 relative z-10 flex items-center justify-center gap-2 font-bold text-sm transition-colors duration-300 ${viewMode === 'grid' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
              <FiGrid className="text-lg" /> Grid View
            </div>
            <div onClick={() => setViewMode('map')} className={`flex-1 relative z-10 flex items-center justify-center gap-2 font-bold text-sm transition-colors duration-300 ${viewMode === 'map' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
        ...
        {error && (
          <div className="absolute top-24 left-1/2 -translate-x-1/2 z-[1000] bg-red-50 text-red-600 px-4 py-2 rounded-lg border border-red-200 shadow-lg text-sm font-medium flex items-center gap-2">
            <FiAlertCircle /> {error}
          </div>
        )}
        ...
        <div className="flex-1 relative bg-gray-50">
          ...
          {viewMode === 'map' && (
            <>
              {mapCenter ? (
                /* FIX: Added absolute inset-0 to force map to fill parent */
                <MapContainer
                  center={mapCenter}
                  zoom={13}
              ...
              {vendors.length > 0 && (
                <div className="absolute bottom-4 left-0 right-0 z-[400] overflow-x-auto px-4 pb-2">
                  <div className="flex gap-4">
                    {vendors.slice(0, visibleVendorsCount).map((vendor) => (
                      <div key={vendor.id} className={`flex-shrink-0 w-72 bg-white rounded-xl shadow-xl p-4 cursor-pointer border transition-all ${selectedVendor?.id === vendor.id ? 'border-orange-500 ring-4 ring-orange-100' : 'border-gray-100'}`} onClick={() => setSelectedVendor(vendor)}>
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h3 className="font-bold text-gray-900 truncate">{vendor.name}</h3>
                            <p className="text-xs text-gray-500">{formatDistance(vendor.distance)} • {formatTimeAgo(vendor.last_updated)}</p>
                          </div>
                          <span className="text-xs font-bold bg-orange-100 text-orange-700 px-2 py-1 rounded-full">Open</span>
                        </div>
                        <div className="flex flex-wrap gap-1 mb-3">
                          {vendor.menu?.items?.slice(0, 2).map((item, idx) => (
                            <span key={idx} className="text-[10px] bg-gray-100 text-gray-600 px-2 py-1 rounded">{item}</span>
                          ))}
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); handleOrderClick(vendor); }} className="w-full py-2 bg-gray-900 text-white text-sm font-bold rounded-lg hover:bg-orange-600 transition-colors">
                          View Menu
                        </button>
                      </div>
                    ))}
            ...
            <div className="h-full overflow-y-auto p-4 sm:p-6">
              {vendors.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-gray-400">
                  ...
                  <p>No vendors found in this area.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {vendors.map((vendor) => (
                    <div key={vendor.id} onClick={() => handleOrderClick(vendor)} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-orange-100 transition-all cursor-pointer overflow-hidden group">
                      <div className="h-32 bg-gray-200 relative overflow-hidden">
                        <div className="absolute inset-0 flex items-center justify-center text-gray-400 bg-gray-100">
                          {vendor.image ? <img src={vendor.image} alt={vendor.name} className="w-full h-full object-cover" /> : <span className="text-sm font-medium">Vendor Image</span>}
                        </div>
                        <div className="absolute top-3 right-3 bg-white/90 backdrop-blur px-2 py-1 rounded-full text-xs font-bold text-orange-700 shadow-sm border border-orange-100">{formatDistance(vendor.distance)}</div>
                      </div>
                      <div className="p-5">
                        <h3 className="font-bold text-lg text-gray-900 mb-1 group-hover:text-orange-600 transition-colors">{vendor.name}</h3>
                        <p className="text-sm text-gray-500 mb-4">Updated {formatTimeAgo(vendor.last_updated)}</p>
                        <div className="space-y-2 mb-4">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {vendor.menu?.items?.slice(0, 3).map((item, idx) => (
                              <span key={idx} className="text-xs bg-orange-50 text-orange-700 border border-orange-100 px-2 py-1 rounded-md">{item}</span>
                            ))}
                          </div>
                        </div>
                        <button className="w-full py-2.5 bg-white border border-gray-200 text-gray-700 font-semibold rounded-xl group-hover:bg-orange-600 group-hover:text-white group-hover:border-orange-600 transition-all">Order Now</button>
                      </div>
                    </div>
                  ))}
                </div>
